...
- [x] `GET /api/games?search=&page=1&per_page=24&filter_type=&filter_value=` — lista paginada de juegos, ordenados por `MAX(release_date)` desc. Tamaño de página configurable vía `per_page` (default 24). Búsqueda por título con `LIKE`. Parámetros opcionales `filter_type` (genre/developer/publisher/theme/game_mode/player_perspective) y `filter_value`. Género filtra por ID; developer/publisher por texto exacto; theme/game_mode/player_perspective con `whereJsonContains`. Responde `{ data, current_page, last_page, total }`.
...
- Filtro IGDB `& status = null` añadido a `search()`, `topByGenre()` y `recentGames()`: en IGDB el campo `status` se omite cuando el juego está lanzado (null = Released); valores no-null indican Alpha (2), Beta (3), Early Access (4), Cancelled (6), etc. Con este filtro solo se importan juegos lanzados (null/0) y Early Access (4); se excluyen Alpha, Beta, Cancelled, Delisted. El campo `first_release_date` es un Unix timestamp que se convierte con `date('Y-m-d', ...)` al almacenar en `Product_x_Platform.release_date`.
- Performance pass: `GameCardComponent` eliminó `@HostListener('window:resize')` (N listeners globales por tarjeta), `@HostListener('mouseenter/leave')` y `truncateName()` — reemplazados por CSS puro (`:hover`, `text-overflow: ellipsis`); filtros de developer/publisher/franchise ahora usan `where()` nativo de Laravel con deslugificación PHP en vez de `LOWER(REPLACE(...))` en SQL, lo que permite uso de índice; migración agrega índices a `developer`, `publisher` y `franchise` en `GameDetails`.
- URLs SEO-friendly para filtros de juegos: `/games/genre/action-rpg`, `/games/developer/naughty-dog`, etc. en lugar de query params. Frontend genera slugs con `slugify()` (normalize NFD + lowercase + hyphens). Backend filtra géneros por `slug` y texto libre con `LOWER(REPLACE(field, ' ', '-')) = ?`; campos JSON (themes, game_modes, player_perspectives) con `JSON_SEARCH(LOWER(col), 'one', ?)`. El endpoint `show()` ahora incluye `slug` en cada género del producto.
- Endpoint `GET /api/products/trailers`: devuelve los últimos 20 productos con `trailer_youtube_id` no nulo, ordenados por `created_at` desc. Usado por la sección "Últimos tráilers" de la landing.
